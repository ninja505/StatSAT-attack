...
namespace ckt_n {
    ...
    {
        using namespace sat_n;
        // std::cout << "Entering init_solver()A of ckt.cpp" << std::endl;
        node2var_map_t node2vars;
        for(unsigned i=0; i != nodes.size(); i++) {
            node2vars[nodes[i]] = Var(i+1);
        }
    ...
    {
        using namespace sat_n;
        std::cout << "Entering init_solver()B of ckt.cpp" << std::endl;
    ...
    {
        using namespace sat_n;
        std::cout << "Entering init_solver()C of ckt.cpp" << std::endl;
        node2var_map_t node2vars;
        for(unsigned i=0; i != nodes.size(); i++) {
            node2vars[nodes[i]] = Var(i+1);
        }
    ...
    {
        using namespace sat_n;
        std::cout << "Entering init_solver()D of ckt.cpp" << std::endl;
        node2var_map_t node2vars;
        for(unsigned i=0; i != nodes.size(); i++) {
            node2vars[nodes[i]] = Var(i+1);
        }
    ...
    {
        using namespace sat_n;
        std::cout << "Entering init_solver()E of ckt.cpp" << std::endl;
        node2var_map_t node2vars;
        for(unsigned i=0; i != nodes.size(); i++) {
            node2vars[nodes[i]] = Var(i+1);
        }
    ...
    {
        using namespace sat_n;
        // std::cout << "Entering init_solver()F of ckt.cpp" << std::endl;
        node2var_map_t node2vars;
        for(unsigned i=0; i != nodes.size(); i++) {
            node2vars[nodes[i]] = Var(i+1);
        }
    ...
    //Ankit - A function to evaluate output given input from the locked circuit
    // fitted with the correct key by sampling one/multiple times 
    // using the actual error rates (when done multiple times)
    std::vector<double> ckt_t::evaluate_sim(const std::vector<bool> input_values, std::vector<bool> key_values, bool sampling, bool est)
    {
        // std::cout<<"Entering evaluate_sim in ckt.cpp\n";
        // std::cout << "Input is: "; bool2hex(input_values);
        // std::cout <<" Key: "<<bool2hex(key_values);
        // std::cout<<"Num gates in locked circuit = "<<gates_sorted.size()<<std::endl;
        for (int i = 0; i < ckt_inputs.size(); ++i)
            ckt_inputs[i]->output_bit = input_values[i];
        for (int i = 0; i < key_inputs.size(); ++i)
            key_inputs[i]->output_bit = key_values[i];

        std::vector<double> mean_output_values(outputs.size(),0);

        int max_iter;
        if(sampling)
            max_iter = IO_sampling_iter;
        else 
            max_iter = 1; // to be used in the case when oracle error is not considered
        ...
        for (int iter = 1; iter <= max_iter; ++iter)
        {
            ...
            {                
                ...
                // flip output with a small probability
                if(sampling)
                {
                    if(est)
                    {
                        if(100*(static_cast<double>(rand()) / RAND_MAX)<=est_error_rate)
                            gate->output_bit = !gate->output_bit;
                    }
                    else
                    {
                        if(100*(static_cast<double>(rand()) / RAND_MAX)<=gate->error_rate)
                        {
                            gate->output_bit = !gate->output_bit;
                        }    
                    }           
            ...
            {
                // std::cout<<outputs[i]->output_bit;
                mean_output_values[i] += (int)(outputs[i]->output_bit);
            }
            // std::cout<<"\n";
    ...
    std::pair<std::vector<double>,std::vector<double>> ckt_t::evaluate_probs(const std::vector<bool> input_values, const std::vector<double> key_values, bool error, bool est)
    {
        ...
        for (auto* gate : gates_sorted )
        {
            ...
            if(error)
            {
                if(est) // use estimated error rate
                {
   ...
   std::pair<std::vector<double>,std::vector<double>> ckt_t::evaluate_probs_as(const std::vector<double> input_signal_probs, const std::vector<double> input_error_probs)
    ...
    void ckt_t::dump_gate_info(node_t* gate)
    {
        ...
        if(gate->num_inputs()>=3)
            std::cout<<" NOTE: "<<gate->name<<" has "<<gate->num_inputs()<<" inputs\n";
            // std::cout<<"\n";
    }

    //Ankit - Function to display info of all gates
    void ckt_t::dump_all_gates_info()
    {
        for (auto* gate : gates_sorted )
            dump_gate_info(gate);
    }

    void ckt_t::get_num_error_gates()
    {
        int num_pos_error_gates=0;
        for(auto* gate: gates_sorted)
            if(gate->error_rate>=0.001)
                num_pos_error_gates += 1;
        std::cout<<"Num pos error gates = "<<num_pos_error_gates<<"\n";
    }
    ...
    {
        // std::cout<<"Entering split_gates() of ckt.cpp\n";
        for(unsigned i=0; i != num_gates(); i++) {
            // std::cout<<"Handling gate "<<gates[i]->name<<"\n";
            _split_gate(gates[i]);
        }
        _init_fanouts();
        _init_indices();
    ...
    void ckt_t::_split_gate(node_t* g)
    {
        ...
        if(g->num_inputs() > 2)
        {
            // std::cout<<"Gate "<<g->name<<" has more than 2 inputs\n";
            if(g->func == "xnor"  || g->func == "nand" || g->func == "nor") {
                ...
                node_t* inp = _create_gate(g->inputs, f, g->name); // Ankit - modified name argument
                inp->error_rate = g->error_rate; // transfer error rate to the inverted gate

                g->func = "not";
                g->name = n;
                g->inputs.resize(1);
                g->inputs[0] = inp;
                g->error_rate = 0; // g(the inverter) would have no error
                g = inp;
            }
    ...
    // JOHANN
    // read the error probabilities of the gates in the circuit
	...
	// parse sampling flag; if true sample multiple output observations for one particular input pattern
	in >> sampling_flag;
	...
	while (!in.eof()) 
    {
		...
		if (gate == nullptr) {
            ...
            if(gate->func == "buf" || gate->func == "BUF")
                gate->error_rate = 0; // Ankit - reset error rates of buffers in simckt

            if(gate->func == "not" || gate->func == "NOT")
                gate->error_rate = 0; // Ankit - reset error rates of inverters in simckt
            ...
            // Ankit - removed support for polymorphic gate
		}
	}
    ...
    // Ankit - get the known correct key for the locked circuit
    void ckt_t::read_known_key(std::string file)
    {
        std::ifstream in;
        std::string specified_key;  // Ankit
        std::string drop;
        in.open(file.c_str());

        if (!in.good()) {
            std::cout << std::endl;
            std::cout << "No known key file: " << file << std::endl;
        ...
        // drop "="
        in >> drop;
        // then read the specified key value
        in >> specified_key;  // Ankit

        assert(num_key_inputs()==specified_key.size());
        for (int i = 0; i < specified_key.size(); ++i)
        {
            if(specified_key[i]=='0')
            {
                known_correct_key.push_back(false);
            }
            else if(specified_key[i]=='1')
                known_correct_key.push_back(true);
            else
            {
                std::cout<<"Specified key has bits other than 0 and 1\n";
                exit(1);
            }
